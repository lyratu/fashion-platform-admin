...
const Wardrobe: React.FC = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'top' | 'bottom' | 'shoes' | 'accessory'>('all');
  const [draggingItem, setDraggingItem] = useState<ClothingItem | null>(null);
  const [activeDrawer, setActiveDrawer] = useState<'top' | 'bottom' | 'shoes' | 'accessory' | null>(null);
  ...
  useEffect(() => {
    ...
    const mockClothingItems: ClothingItem[] = [
      { id: 't1', name: '白色T恤', type: 'top', imageUrl: 'https://via.placeholder.com/150?text=白色T恤', season: 'summer', tags: ['休闲', '基础款'] },
      { id: 't2', name: '黑色衬衫', type: 'top', imageUrl: 'https://via.placeholder.com/150?text=黑色衬衫', season: 'all', tags: ['正式', '百搭'] },
      { id: 't3', name: '蓝色卫衣', type: 'top', imageUrl: 'https://via.placeholder.com/150?text=蓝色卫衣', season: 'autumn', tags: ['休闲', '保暖'] },
      { id: 't4', name: '条纹衬衫', type: 'top', imageUrl: 'https://via.placeholder.com/150?text=条纹衬衫', season: 'spring', tags: ['商务', '时尚'] },
      { id: 'b1', name: '牛仔裤', type: 'bottom', imageUrl: 'https://via.placeholder.com/150?text=牛仔裤', season: 'all', tags: ['休闲', '百搭'] },
      { id: 'b2', name: '黑色西裤', type: 'bottom', imageUrl: 'https://via.placeholder.com/150?text=黑色西裤', season: 'all', tags: ['正式', '商务'] },
      { id: 'b3', name: '卡其短裤', type: 'bottom', imageUrl: 'https://via.placeholder.com/150?text=卡其短裤', season: 'summer', tags: ['休闲', '舒适'] },
      { id: 'b4', name: '灰色运动裤', type: 'bottom', imageUrl: 'https://via.placeholder.com/150?text=灰色运动裤', season: 'all', tags: ['运动', '舒适'] },
      { id: 's1', name: '白色运动鞋', type: 'shoes', imageUrl: 'https://via.placeholder.com/150?text=白色运动鞋', season: 'all', tags: ['休闲', '百搭'] },
      { id: 's2', name: '黑色皮鞋', type: 'shoes', imageUrl: 'https://via.placeholder.com/150?text=黑色皮鞋', season: 'all', tags: ['正式', '商务'] },
      { id: 's3', name: '棕色靴子', type: 'shoes', imageUrl: 'https://via.placeholder.com/150?text=棕色靴子', season: 'autumn', tags: ['休闲', '保暖'] },
      { id: 'a1', name: '黑色腰带', type: 'accessory', imageUrl: 'https://via.placeholder.com/150?text=黑色腰带', season: 'all', tags: ['百搭', '基础款'] },
      { id: 'a2', name: '银色项链', type: 'accessory', imageUrl: 'https://via.placeholder.com/150?text=银色项链', season: 'all', tags: ['时尚', '点缀'] },
      { id: 'a3', name: '棒球帽', type: 'accessory', imageUrl: 'https://via.placeholder.com/150?text=棒球帽', season: 'all', tags: ['休闲', '运动'] },
    ];
  ...
  const getCurrentWeather = () => {
    return weatherData.find(w => w.date.toDateString() === selectedDate.toDateString());
  };
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, dropZone: 'top' | 'bottom' | 'shoes' | 'accessory') => {
    e.preventDefault();
    e.currentTarget.classList.remove('bg-blue-50');
    ...
    // 检查拖拽的衣物类型是否与放置区域匹配
    if (draggingItem.type !== dropZone) {
      showToast(`这件衣物不能放在${
        dropZone === 'top' ? '上衣' :
        dropZone === 'bottom' ? '裤子' :
        dropZone === 'shoes' ? '鞋子' : '配饰'
      }区域`, 'error');
      return;
    }
    ...
    if (dropZone === 'accessory') {
      // 配饰可以有多个
      if (!currentOutfit.accessories.some(a => a.id === draggingItem.id)) {
        setCurrentOutfit(prev => ({
          ...prev,
          accessories: [...prev.accessories, draggingItem]
        }));
      }
    } else {
      // 其他类型只能有一个
  ...
  // 移除已选择的衣物
  const handleRemoveItem = (type: 'top' | 'bottom' | 'shoes', itemId: string) => {
    setCurrentOutfit(prev => ({
      ...prev,
      [type]: undefined
    }));
  ...
  // 切换抽屉
  const toggleDrawer = (drawer: 'top' | 'bottom' | 'shoes' | 'accessory') => {
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row mb-8">
          ...
          {isDIYMode ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  >
                    {currentOutfit.top ? (
                      <div className="relative w-full h-full p-2">
                        ...
                        <p className="text-center mt-1">{currentOutfit.top.name}</p>
                      </div>
                    ) : (
                  ...
                  >
                    {currentOutfit.bottom ? (
                      <div className="relative w-full h-full p-2">
                        ...
                        <p className="text-center mt-1">{currentOutfit.bottom.name}</p>
                      </div>
                    ) : (
                  ...
                  >
                    {currentOutfit.shoes ? (
                      <div className="relative w-full h-full p-2">
                        ...
                        <p className="text-center mt-1">{currentOutfit.shoes.name}</p>
                      </div>
                    ) : (
                    ...
                    {currentOutfit.accessories.length > 0 ? (
                      <div className="flex flex-wrap justify-center gap-2 p-2 w-full">
                        {currentOutfit.accessories.map(accessory => (
                          <div key={accessory.id} className="relative w-24 h-24">
                            <img
                            ...
                            <p className="text-center text-xs mt-1">{accessory.name}</p>
                          </div>
                        ))}
                      </div>
                    ) : (
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="mb-4">
                  <p className="text-gray-700 mb-2">日期：{selectedDate.toLocaleDateString()}</p>
                  <p className="text-gray-700 mb-2">
                    天气：{getCurrentWeather()?.icon} {getCurrentWeather()?.condition} {getCurrentWeather()?.temperature}°C
                  </p>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setCurrentOutfit(prev => ({ ...prev, note: e.target.value }))}
                  />
                </div>
            ...
            <div className="flex items-center justify-center h-64">
              <p className="text-gray-500">请选择"帮我搭配"或"搭配DIY"开始使用</p>
            </div>
          )}
        </div>
        <div className="lg:w-1/4 lg:ml-4 mt-4 lg:mt-0">
          ...
          <div className="bg-white p-4 rounded shadow mb-4">
            ...
            >
              {dates.map((date) => {
                const weather = weatherData.find(w => w.date.toDateString() === date.toDateString());
                return (
                  <div
                  ...
                  >
                    ...
                    <p className="text-center text-sm">
                      {weather ? `${weather.icon} ${weather.temperature}°C` : '加载中...'}
                    </p>
                  </div>
                );
              })}
      ...
      {/* 我的衣柜模块 */}
      <div className="bg-gradient-to-r from-gray-100 to-gray-200 p-8 rounded-xl shadow-2xl mb-8">
        ...
        <div className="flex space-x-4 mb-6 overflow-x-auto pb-2 custom-scrollbar">
          {['全部', '上衣', '裤子', '鞋子', '配饰'].map((category, index) => (
            <button
              key={category}
              onClick={() => setSelectedCategory(
                ...
                index === 1 ? 'top' :
                index === 2 ? 'bottom' :
                index === 3 ? 'shoes' : 'accessory'
              )}
              className={`px-6 py-3 rounded-lg font-medium transition-all duration-300
                ${selectedCategory === (index === 0 ? 'all' :
                  index === 1 ? 'top' :
                  index === 2 ? 'bottom' :
                  index === 3 ? 'shoes' : 'accessory')
          ...
          {filteredClothingItems.map(item => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-white to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex justify-center">
                <span className="text-xs text-blue-600 font-medium">拖拽到搭配区</span>
              </div>
            </div>
          ))}
        </div>
